"""
SignalGenerator (Módulo 7 — Plan III).

Encapsula la lógica de las cinco reglas técnicas que el instructivo exige:
...
El método agregador `.evaluate_all()` retorna la lista completa de señales
generadas con sus campos: `id`, `rule`, `type` (buy/sell), `value`, `msg`
e `interpretation` en lenguaje simple, sin recomendaciones absolutas.

La clase recibe los dos últimos registros técnicos (último y penúltimo) del
endpoint M1 y los umbrales configurables del usuario. Es pura (sin I/O ni
estado global) — la persistencia en `signals_log` la maneja el endpoint.
"""
...
class SignalGenerator:
    ...
    Espera dos dicts con campos producidos por `_build_technical_records`:
    `RSI`, `MACD_Line`, `MACD_Signal`, `MACD_Hist`, `Close`, `BB_Upper`,
    `BB_Lower`, `SMA_20`, `SMA_50`, `Stoch_K`, `Stoch_D`.
    """
    ...
    def macd_cross(self) -> list[dict]:
        ...
        # Fallback al histograma si no hay datos de líneas separadas
        if ml == 0 and sl == 0 and pml == 0 and psl == 0:
            ...
            if mh > 0 and mhp <= 0:
                return [self._signal(
                    ...
                    "El histograma MACD acaba de cruzar hacia territorio positivo, lo que "
                    "indica que el momentum de corto plazo supera al de largo plazo. Esta "
                    "señal técnica puede sugerir un arranque alcista, pero debe evaluarse "
                    "junto con otros indicadores y el contexto de riesgo.",
                )]
            if mh < 0 and mhp >= 0:
                return [self._signal(
                    "MACD_BEAR_CROSS", "MACD cross", "sell", mh,
                    "CRUCE BAJISTA — Histograma MACD negativo",
                    "El histograma MACD cruzó hacia territorio negativo. El momentum de "
                    "corto plazo cedió frente al de largo plazo, lo que puede anticipar "
                    "una corrección o desaceleración del precio.",
                )]
            return []
        ...
        if ml > sl and pml <= psl:
            return [self._signal(
                ...
                "La línea MACD cruzó al alza la línea de señal. Esta es una señal técnica "
                "de momentum positivo de corto plazo, pero no constituye recomendación; "
                "debe confirmarse con otros indicadores y el contexto.",
            )]
        if ml < sl and pml >= psl:
            return [self._signal(
                "MACD_BEAR_CROSS", "MACD cross", "sell", ml - sl,
                f"CRUCE BAJISTA — Línea MACD ({ml:.3f}) cruza por debajo de señal ({sl:.3f})",
                "La línea MACD cruzó a la baja la línea de señal. Puede sugerir pérdida "
                "de momentum, pero no es una recomendación garantizada; evalúe el "
                "contexto de mercado y otros indicadores.",
            )]
        return []

    # ── Regla 2: RSI en zonas extremas ───────────────────────────────────────
    ...
    # ── Regla 3: Bandas de Bollinger ─────────────────────────────────────────

    def bollinger_touch(self) -> list[dict]:
        """Detecta precio tocando o cruzando banda superior/inferior."""
        ...
        if upper and close >= upper:
            out.append(self._signal(
                ...
                "El precio tocó o superó la banda superior de Bollinger (~2σ sobre la "
                "media de 20 días). Estadísticamente puede sugerir extensión al alza y "
                "posible reversión a la media; no es recomendación de venta absoluta.",
            ))
        if lower and close <= lower:
            out.append(self._signal(
                "BB_LOWER_TOUCH", "Bollinger touch", "buy", close,
                f"BANDA INFERIOR — Precio {close:.2f} <= banda inferior ({lower:.2f})",
                "El precio tocó o perforó la banda inferior de Bollinger, cotizando muy "
                "por debajo de su rango habitual. Puede sugerir oportunidad si los "
                "fundamentos del activo no han cambiado, pero también puede anticipar "
                "continuación bajista; no es recomendación garantizada.",
            ))
        return out

    # ── Regla 4: cruce de medias móviles (Golden / Death) ───────────────────

    def moving_average_cross(self) -> list[dict]:
        """Detecta Golden cross (SMA20 cruza por encima de SMA50) o Death cross."""
        ...
        if s_short > s_long and p_short <= p_long:
            return [self._signal(
                ...
                "Este patrón se asocia históricamente con inicios de tendencia alcista "
                "de mediano plazo, pero debe interpretarse junto con volumen y otros "
                "indicadores; no constituye recomendación segura.",
            )]
        if s_short < s_long and p_short >= p_long:
            return [self._signal(
    ...
    def _signal(rule_id: str, rule: str, kind: str, value: float,
                msg: str, interpretation: str) -> dict:
        """Construye el dict de una señal individual."""
        return {
            "id":             rule_id,
            "rule":           rule,
            "type":           kind,
